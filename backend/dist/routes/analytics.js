...
router.get('/emotion-trends', async (req, res) => {
    try {
        ...
        Object.keys(trends).forEach(date => {
            const dayCheckins = recentCheckins.filter(c => new Date(c.created_at).toISOString().split('T')[0] === date);
...
router.get('/garden-health', async (req, res) => {
    try {
        ...
        const plants = await database_service_sqlite_1.dbStatements.getPlantsByUserId(userId);
        const checkins = await database_service_sqlite_1.dbStatements.getCheckinsByUserId(userId);
        ...
        if (checkins.length >= 10) {
            ...
            if (last5Avg > prev5Avg + 0.1)
                healthTrend = 'improving';
            else if (last5Avg < prev5Avg - 0.1)
                healthTrend = 'declining';
        }
        res.json({
            gardenHealth: (garden === null || garden === void 0 ? void 0 : garden.health) || 0,
...
router.get('/achievements', async (req, res) => {
    try {
        ...
        if (checkins.length >= 1)
            achievements.push({ id: 'first_checkin', name: 'First Steps', description: 'Made your first emotional check-in' });
        if (checkins.length >= 10)
            achievements.push({ id: 'regular_logger', name: 'Regular Logger', description: 'Made 10 check-ins' });
        if (checkins.length >= 50)
            achievements.push({ id: 'dedicated_gardener', name: 'Dedicated Gardener', description: 'Made 50 check-ins' });
        if (checkins.length >= 100)
            achievements.push({ id: 'master_gardener', name: 'Master Gardener', description: 'Made 100 check-ins' });
        // Emotional variety achievements
        const emotions = new Set(checkins.map(c => c.emotionLabel));
        if (emotions.size >= 3)
            achievements.push({ id: 'emotion_explorer', name: 'Emotion Explorer', description: 'Experienced 3 different emotions' });
        if (emotions.size >= 5)
            achievements.push({ id: 'emotion_master', name: 'Emotion Master', description: 'Experienced 5 different emotions' });
        // Positive streak achievements
        const positiveCheckins = checkins.filter(c => (c.sentimentScore || 0) > 0.2);
        if (positiveCheckins.length >= 7)
            achievements.push({ id: 'positive_week', name: 'Positive Week', description: '7 positive check-ins in a row' });
        // Plant achievements
        if (plants.length >= 5)
            achievements.push({ id: 'growing_garden', name: 'Growing Garden', description: 'Grew 5 plants' });
        if (plants.length >= 20)
            achievements.push({ id: 'flourishing_garden', name: 'Flourishing Garden', description: 'Grew 20 plants' });
        // Health achievements
        if (garden && garden.health >= 0.8)
            achievements.push({ id: 'healthy_garden', name: 'Healthy Garden', description: 'Maintained garden health above 80%' });
        // Streak achievements
        let currentStreak = 0;
        ...
        if (currentStreak >= 7)
            achievements.push({ id: 'week_streak', name: 'Week Streak', description: 'Checked in for 7 days straight' });
        if (currentStreak >= 30)
            achievements.push({ id: 'month_streak', name: 'Month Streak', description: 'Checked in for 30 days straight' });
        res.json({
            achievements,
            stats: {
                totalCheckins: checkins.length,
...
function generateRecommendations(avgSentiment, dominantEmotion, trend) {
