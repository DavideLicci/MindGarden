...
router.get('/:plantId', async (req, res) => {
    try {
        ...
        const plant = await db_1.dbStatements.getPlantById(plantId);
        if (!plant) {
...
router.post('/:plantId/actions', async (req, res) => {
    try {
        const { plantId } = req.params;
        ...
        await db_1.dbStatements.updatePlantHealth(plantId, newHealth, newGrowth);
        // Return updated plant
        const updatedPlant = await db_1.dbStatements.getPlantById(plantId);
